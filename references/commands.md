...
**新手友好**：`run` 的任务描述处输入序号（如 `1`）可直接使用内置示例模板——办公场景（企业微信回消息/钉钉打卡/查收邮件/OA 审批）演示"AI 替你干活"，生活场景（看桌面/发微信/刷抖音/找餐厅/查天气）快速上手，带 `*` 的示例需云手机预装对应 App；首次运行显示欢迎页；报错输出大白话"发生了什么/你该怎么办"；任务结束明确提示完成状态与用时。
