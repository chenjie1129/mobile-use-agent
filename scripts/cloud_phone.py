...
# Modified in 2026 by the mobile-use-agent contributors to add policy-gated calls.
"""
火山云手机 OpenAPI 客户端
"""
...
from control_policy import authorize_action
...
def redact_sensitive_response(value: Any) -> Any:
    """Redact credentials and signed URLs before returning data to an Agent."""
    if isinstance(value, dict):
        return {
            key: (
                "[REDACTED]"
                if key.lower().replace("_", "") in SENSITIVE_RESPONSE_KEYS
                else redact_sensitive_response(item)
            )
            for key, item in value.items()
        }
    if isinstance(value, list):
        return [redact_sensitive_response(item) for item in value]
    return value
...
class VePhoneClient:
    ...
    def controlled_action(
        ...
        *,
        ...
        """Invoke any supported ACEP action behind the risk policy."""
        authorize_action(action, confirmation_token)
        if action not in self.ACTION_VERSIONS:
            raise ValueError(f"Unsupported ACEP action: {action}")
        return redact_sensitive_response(
            self.request_action(
                action,
                json_body=json_body,
                **(params or {}),
