...
- **回客户消息**：`打开企业微信，把最新一条客户消息回复"收到，今天下班前给您反馈"`
- **上下班打卡**：`打开钉钉，完成上班打卡，并告诉我打卡结果`
- **查收邮件并汇报**：`打开邮箱，查看最新一封未读邮件，告诉我发件人、主题和一句话摘要`
- **处理审批**：`打开 OA 系统，把待审批的请假申请都通过掉`
...
> **新手小贴士**
>
> - 不知道让它做什么？在"请描述任务"处输入序号（如 `1`），直接用现成示例：**办公场景**（企业微信回消息、钉钉打卡、查收邮件、OA 审批）能直观展示"AI 替你干活"；**生活场景**（看桌面、发微信、刷抖音、找餐厅、查天气）让新手快速上手。带 `*` 的示例需要云手机上已安装对应 App。
...
MUA_TOOL_SCHEMA = {
    ...
    "function": {
        ...
        "description": "在火山引擎云手机上执行真实手机操作（回客户消息、打卡、查邮件、处理审批等办公任务，也可搜索、下单），返回任务结果",
        "parameters": {
            "type": "object",
            "properties": {
                "prompt": {"type": "string", "description": "自然语言描述想执行的操作，如「打开企业微信回复最新客户消息」"},
